        ...
        <div className='flex gap-2 items-center overflow-x-scroll scrollbar-none' ref={scrollElement}>
            ...
            <button className='bg-white rounded-full p-1 absolute right-0 hidden md:block' onClick={scrollRight}><FaAngleRight/></button>
            {
                products.map((product, index)=>{
                    return(
                        <div key={index} className='bg-white w-full min-w-[140px] md:min-w-[240px] max-w-[280px] md:max-w-[320px] rounded-sm shadow'>
                            <div className='bg-slate-200 p-2 min-w-[120px] md:min-w-[145px] h-32'>
                                <img src={product?.productImage[0]} alt={product?.productName} className='w-full h-full object-scale-down hover:scale-110 transition-all mix-blend-multiply'/>
                            </div>
                            <div>
                                ...
                                <button className='px-4 text-white bg-red-600 rounded-full py-1 hover:bg-red-700 transition-all mx-2 my-2'>Add to Cart</button>
                            </div>
                        </div>
                    )
                })
